...
export type AvatarUploadStatus = 'idle' | 'loading' | 'saved';
...
export function PersonalInfoPhotoSection({
  ...
  const isBusy = isLoading || isSaved;

  const pillLabel = isLoading ? 'Saving…' : isSaved ? 'Saved' : actionLabel;
  const pillBackground = isSaved ? '#E4F4E8' : '#D3E9FA';
  const pillColor = isSaved ? '#3D7A4A' : '#4D7A9A';
        ...
        <View style={{ width: AVATAR_SIZE, height: AVATAR_SIZE }}>
          ...
          <View
            style={{
              ...
              backgroundColor: isSaved ? '#E4F4E8' : '#FFFFFF',
              borderWidth: 2,
              borderColor: '#F9F9F9',
              alignItems: 'center',
              justifyContent: 'center',
            ...
            {isSaved ? (
              <IconsaxTickCircleIcon size={18} color="#65D900" />
            ) : isLoading ? null : (
      ...
      <View style={{ alignItems: 'center', gap: 12, maxWidth: 280 }}>
        <Pressable
          ...
          style={({ pressed }) => ({
            ...
            opacity: isLoading ? 0.75 : pressed && !isBusy ? 0.88 : 1,
          })}>
          <Text
            style={{
        ...
        <Text
          style={{
            ...
            fontSize: 13,
            color: isSaved ? '#3D7A4A' : 'rgba(114, 114, 114, 0.62)',
            letterSpacing: -0.2,
            lineHeight: 18,
            textAlign: 'center',
          }}>
